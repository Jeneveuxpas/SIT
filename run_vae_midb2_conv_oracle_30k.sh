#!/usr/bin/env bash
# Train and evaluate the contextual VAE mid.block_2 convolutional K/V oracle.
set -euo pipefail

EXP_NAME="sit-xl2-attnscaf2-vaemidb2-conv-s8-t30k-oracle"
...
if [[ -f "$CHECKPOINT" && "${FORCE_TRAIN:-false}" != "true" ]]; then
  ...
  echo "Training contextual VAE-feature oracle: ${EXP_NAME} (${WORLD_SIZE} GPU(s))"
