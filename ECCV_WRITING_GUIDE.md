...
Self-attention is the central mechanism by which diffusion transformers route information across image regions, yet at initialisation the keys and values are random, leaving attention near-uniform and forcing the model to discover locality from scratch—a bottleneck we term _routing cold start_. Representation-alignment methods accelerate training by matching hidden states to pretrained encoders, but recent analyses show that their effective ingredient is spatial structure rather than semantics, and that the benefit plateaus or degrades over time. We propose **Attention Scaffolding**, which supplies spatial priors directly at the operator level: keys and values from a frozen encoder are temporarily injected into selected self-attention layers, bootstrapping coherent routing. The scaffold is then removed and the induced patterns distilled into the model's own parameters, adding no inference cost. On ImageNet 256×256, SiT-XL/2 with Attention Scaffolding matches vanilla-baseline quality in 35× fewer steps and  the best reported representation-alignment result with approximately 5× less compute and finally achieves [best FID] with approximately 5× less compute than representation-alignment methods.

Self-attention is the central mechanism by which diffusion transformers route information across image regions, yet at initialisation it carries no spatial prior: the keys and values that determine where and what information is routed are randomly initialised, leaving attention near-uniform and forcing the model to discover locality entirely from data—a bottleneck we term routing cold start. Representation-alignment methods such as REPA accelerate training by matching denoising hidden states to pretrained encoder features, but two recent findings expose the limits of this feature-level strategy: the effective ingredient is spatial structure rather than semantic content, and the alignment benefit plateaus or even degrades in later training. Together, these observations suggest that spatial priors should be supplied directly to the attention operator rather than passed indirectly through hidden-state matching. We propose Attention Scaffolding, a training-time mechanism that injects keys and values from a frozen vision encoder into selected self-attention layers, giving the diffusion transformer's queries an immediate spatial routing signal. Once coherent locality emerges, the scaffold is removed and the induced routing patterns are distilled into the model's native key–value parameters—incurring zero additional cost at inference. On ImageNet 256×256, Attention Scaffolding enables SiT-XL/2 to match vanilla-baseline quality in 35× fewer steps and to reach FID 5.7 at 740K iterations, surpassing the best reported representation-alignment result with approximately 5× less compute. These results suggest that operator-level routing priors offer a more direct and efficient path to accelerating diffusion transformer training, complementary to feature-space alignment.
