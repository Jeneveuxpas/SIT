#!/usr/bin/env bash
# Train/evaluate paired VAE mid.block_2 + three-layer K/V projector oracle.
set -euo pipefail

EXP_NAME="sit-b2-attnscaf2-vaemidb2-convmlp3-s6-t30k-oracle"
...
if [[ -f "$CHECKPOINT" && "${FORCE_TRAIN:-false}" != "true" ]]; then
  ...
  echo "Training VAE mid.block_2 conv+MLP3 oracle: ${EXP_NAME} (${WORLD_SIZE} GPU(s))"
