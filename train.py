...
def parse_args(input_args=None):
    ...
    parser.add_argument("--kv-proj-type", type=str, default="linear",
                        choices=["linear", "mlp", "conv", "conv_mlp3", "conv_mlp5", "head_gate"],
                        help="Projection type for Encoder K/V: linear, mlp, conv, conv_mlp3, conv_mlp5, or head_gate")
    ...
    if args.kv_proj_stride != 1 and args.kv_proj_type not in ("conv", "conv_mlp3", "conv_mlp5"):
        parser.error(
            "--kv-proj-stride other than 1 requires a convolutional KV projector"
        )
    ...
    if args.scaffold_feature_source == "vae_mid_block2":
        ...
        if args.kv_proj_type not in ("conv", "conv_mlp3", "conv_mlp5") or args.kv_proj_stride != 2:
            parser.error(
                "--scaffold-feature-source vae_mid_block2 requires "
                "--kv-proj-type conv/conv_mlp3/conv_mlp5 and --kv-proj-stride 2"
            )
    if (
