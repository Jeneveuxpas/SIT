"""
Training script for SiT with DINO-KV Distillation (sota iREPA version).

Two-stage training:
- Stage 1: Q_SiT @ K_DINO, V_DINO (linear projection trainable)
- Stage 2: Q_SiT @ K_SiT, V_SiT + logits distillation (projection detached)

Based on sota/iREPA train.py with DINO-KV integration.
"""
import argparse
import copy
...
from models.sit_dinokv import SiT_DINOKV_models, DinoKVExtractor
from loss_dinokv import SILossWithDINOKV
...
def preprocess_for_dino(x, resolution=256):
    """Preprocess raw images for DINOv2."""
...
def safe_unwrap_model(model, compiled=False):
    """
    Safely unwrap model from DDP and torch.compile wrappers.
    This avoids issues with accelerate's unwrap_model detection.
    """
    # Unwrap DDP
...
def parse_layer_indices(indices_str: str) -> list:
    """Parse comma-separated layer indices string to list of ints."""
...
def main(args):    
    ...
    # Enable find_unused_parameters for stage transition (Stage 1 → Stage 2)
    # This is required because different parameters are used in different stages
    # (DINO K/V projections in Stage 1 vs SiT K/V in Stage 2)
    ...
    # Parse layer indices
    dino_layer_indices = parse_layer_indices(args.dino_layer_indices)
    sit_layer_indices = parse_layer_indices(args.sit_layer_indices)
    assert len(dino_layer_indices) == len(sit_layer_indices), \
        "DINO and SiT layer indices must have same length"
    ...
    # Create DINO K/V extractor
    dino_encoder = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitb14')
    dino_encoder = dino_encoder.to(device)
    dino_kv_extractor = DinoKVExtractor(dino_encoder, dino_layer_indices)
    dino_kv_extractor.eval()
    ...
    if accelerator.is_main_process:
        ...
        logger.info(f"DINO-KV: {len(dino_layer_indices)} layer pairs")
        logger.info(f"DINO layers: {dino_layer_indices} -> SiT layers: {sit_layer_indices}")
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_dataloader:
            ...
            with torch.no_grad():
                ...
                # Extract DINO K/V and CLS token
                raw_image_dino = preprocess_for_dino(raw_image, args.resolution)
                with accelerator.autocast():
                    dino_kv_list, dino_cls = dino_kv_extractor(raw_image_dino)
            ...
            with accelerator.accumulate(model):
                ...
                if accelerator.sync_gradients and global_step % args.ema_update_freq == 0:
                    original_model = safe_unwrap_model(model, compiled=args.compile)
                    update_ema(
                        ema,
                        original_model,
            ...
            # Use original sampler for sampling
    ...
    # Cleanup
    dino_kv_extractor.remove_hooks()
...
def parse_args(input_args=None):
    parser = argparse.ArgumentParser(description="Training SiT with DINO-KV")
    ...
    # model
    parser.add_argument("--model", type=str, default="SiT-B/2-DINOKV")
    ...
    parser.add_argument("--dino-layer-indices", type=str, default="8",
                        help="Comma-separated DINO layer indices for K/V extraction")
    parser.add_argument("--sit-layer-indices", type=str, default="4",
                        help="Comma-separated SiT layer indices for K/V injection")
    parser.add_argument("--stage1-ratio", type=float, default=0.3,
                        help="Ratio of training for Stage 1 (DINO K/V). e.g., 0.5 = first 50%")
