#!/usr/bin/env bash
# Train SiT-B/2 with the contextual VAE feature oracle, then evaluate 10K samples.
set -euo pipefail

EXP_NAME="sit-b2-attnscaf2-vaemidb2-conv-s6-t30k-oracle"
...
if [[ -f "$CHECKPOINT" && "${FORCE_TRAIN:-false}" != "true" ]]; then
  ...
  echo "Training SiT-B/2 contextual VAE-feature oracle: ${EXP_NAME} (${WORLD_SIZE} GPU(s))"
