#!/usr/bin/env bash
# Paired VAE oracle: non-overlapping k2/s2 spatial mapping + MLP5 K/V.
set -euo pipefail

EXP_NAME="sit-b2-attnscaf2-vaemidb2-k2-convmlp5-s6-t30k-oracle"
...
if [[ -f "$CHECKPOINT" && "${FORCE_TRAIN:-false}" != "true" ]]; then
  ...
  echo "Training VAE k2/s2 conv+MLP5 oracle: ${EXP_NAME} (${WORLD_SIZE} GPU(s))"
