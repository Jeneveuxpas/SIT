#!/usr/bin/env bash
# Train the VAE-attention K/V Stage-1 oracle, then run paired-reference FID.
set -euo pipefail

EXP_NAME="sit-xl2-attnscaf2-vaekv-s8-t30k-oracle"
CONFIG="configs/${EXP_NAME}.yaml"
GPU_LIST="${GPU:-0,1}"
WORLD_SIZE="${NUM_GPUS:-2}"
FID_SAMPLES="${NUM_FID_SAMPLES:-50000}"
...
if [[ -f "$CHECKPOINT" && "${FORCE_TRAIN:-false}" != "true" ]]; then
  ...
  echo "Training paired VAE-K/V oracle: ${EXP_NAME} (${WORLD_SIZE} GPU(s))"
