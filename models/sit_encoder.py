...
class AttentionWithEncoderKV(nn.Module):
    ...
    def forward(
        ...
        kv_replace_mode controls which components are replaced from encoder:
        - 'kv': Replace K,V (default)
        - 'k': Replace only K
        - 'v': Replace only V
        - 'qkv': Replace Q,K,V
        - 'q': Replace only Q
        ...
        # Apply QK norm
        q_sit = self.q_norm(q_sit)
        k_sit = self.k_norm(k_sit)
        
        distill_loss = None
        
        # Helper: select Q/K/V based on kv_replace_mode and encoder outputs
        def _select_qkv(q_src, k_src, v_src):
            """Select which components to use from encoder vs SiT based on mode."""
            q = q_src if (kv_replace_mode in ('qkv', 'q') and q_src is not None) else q_sit
            k = k_src if (kv_replace_mode in ('kv', 'k', 'qkv') and k_src is not None) else k_sit
            v = v_src if (kv_replace_mode in ('kv', 'v', 'qkv') and v_src is not None) else v_sit
            return q, k, v
        ...
        if stage == 1 and has_enc:
            ...
            if align_mode == 'logits':
                logits_enc = (q_teacher.float() @ k_teacher.float().transpose(-2, -1)) * self.scale
                logits_sit = (q_sit.float() @ k_sit.float().transpose(-2, -1)) * self.scale
                ...
                loss_components = []
                if k_enc is not None and kv_replace_mode in ('kv', 'k', 'qkv'):
                ...
                if q_enc is not None and kv_replace_mode in ('qkv', 'q'):
                ...
                if loss_components:
                    ...
                    distill_loss = (kv_weight * combined).mean()
                else:
                ...
                attn_sit = self._compute_attn_output(q_sit, k_sit, v_sit)
                attn_loss_per_sample = self._per_sample_mse(attn_sit, attn_enc.detach())
                # Component-wise MSE for replaced components
                comp_loss_components = []
                if k_enc is not None and kv_replace_mode in ('kv', 'k', 'qkv'):
                    comp_loss_components.append(self._per_sample_mse(k_sit.float(), k_enc.float().detach()))
                if v_enc is not None and kv_replace_mode in ('kv', 'v', 'qkv'):
                    comp_loss_components.append(self._per_sample_mse(v_sit.float(), v_enc.float().detach()))
                if q_enc is not None and kv_replace_mode in ('qkv', 'q'):
                    comp_loss_components.append(self._per_sample_mse(q_sit.float(), q_enc.float().detach()))
                comp_loss = sum(comp_loss_components) if comp_loss_components else torch.zeros_like(attn_loss_per_sample)
                kv_weight = self._snr_weight(
                    time_input=time_input,
                    path_type=path_type,
                    dtype=attn_loss_per_sample.dtype,
                ...
                distill_loss = (
                    ...
                    + self.kv_loss_weight * kv_weight * comp_loss
                ).mean()
...
class SiTBlockWithEncoderKV(nn.Module):
    ...
    def forward(
        ...
        x: torch.Tensor, 
        c: torch.Tensor,
        enc_kv: Optional[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]] = None,
        stage: int = 2,
        align_mode: str = 'attn_mse',
        time_input: Optional[torch.Tensor] = None,
        path_type: str = "linear",
