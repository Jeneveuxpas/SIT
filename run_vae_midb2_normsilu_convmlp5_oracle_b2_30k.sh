#!/usr/bin/env bash
# Train/evaluate norm_out+SiLU VAE source with k3 MLP5 and no final K/V norm.
set -euo pipefail

EXP_NAME="sit-b2-attnscaf2-vaemidb2-normsilu-convmlp5-s6-t30k-oracle"
...
if [[ -f "$CHECKPOINT" && "${FORCE_TRAIN:-false}" != "true" ]]; then
  ...
  echo "Training norm_out+SiLU VAE K/V oracle: ${EXP_NAME} (${WORLD_SIZE} GPU(s))"
