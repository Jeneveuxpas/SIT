#!/usr/bin/env bash
# Train/evaluate the combined norm_out+SiLU, MLP5, final-K/V-norm oracle.
set -euo pipefail

EXP_NAME="sit-b2-attnscaf2-vaemidb2-normsilu-convmlp5-postkvnorm-s6-t30k-oracle"
...
if [[ -f "$CHECKPOINT" && "${FORCE_TRAIN:-false}" != "true" ]]; then
  ...
  echo "Training combined VAE K/V oracle: ${EXP_NAME} (${WORLD_SIZE} GPU(s))"
